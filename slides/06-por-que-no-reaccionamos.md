---
layout: section
---

# ¿Por qué no reaccionamos?

<!--
"Llevamos más de una hora juntos. Les he contado que la IA clona voces, manipula elecciones, contribuyó a la muerte de una persona, resiste su apagado, chantajea, finge portarse bien.

Y sin embargo, una parte de ustedes sigue pensando: 'Bueno, sí, pero tampoco será para tanto.'

Eso es normal. Y tiene nombre."
-->

---

# Sesgos cognitivos

<div class="grid grid-cols-2 gap-6 mt-8">
  <div v-click class="p-6 rounded-xl bg-white/5">
    <div class="text-2xl font-bold text-orange-400 mb-2">Sesgo de normalidad</div>
    <div class="opacity-70">"Nunca nos ha pasado, así que no nos pasará."</div>
    <div class="opacity-50 text-sm mt-2">Es lo que hace que la gente no evacúe con alerta de tsunami.</div>
  </div>
  <div v-click class="p-6 rounded-xl bg-white/5">
    <div class="text-2xl font-bold text-orange-400 mb-2">Ceguera exponencial</div>
    <div class="opacity-70">Nuestro cerebro piensa en línea recta.</div>
    <div class="opacity-50 text-sm mt-2">La IA se duplica.</div>
  </div>
  <div v-click class="p-6 rounded-xl bg-white/5">
    <div class="text-2xl font-bold text-orange-400 mb-2">Insensibilidad al alcance</div>
    <div class="opacity-70">Una muerte nos conmueve. 8.000 millones en riesgo nos suena a estadística.</div>
  </div>
  <div v-click class="p-6 rounded-xl bg-white/5">
    <div class="text-2xl font-bold text-orange-400 mb-2">Ciclo risa-negación-miedo</div>
    <div class="opacity-70">Primero se ríen. Luego niegan. Solo temen después de mucha reflexión.</div>
  </div>
</div>

<!--
"Sesgo de normalidad: 'Nunca nos ha pasado, así que no nos pasará.' Es lo que hace que la gente no evacúe con alerta de tsunami.

Ceguera exponencial: Nuestro cerebro piensa en línea recta. La IA se duplica.

Insensibilidad al alcance: Una muerte nos conmueve. Ocho mil millones de vidas en riesgo nos suena a estadística.

Ciclo risa-negación-miedo: Primero se ríen. Luego niegan. Solo temen después de mucha reflexión."
-->

---
layout: center
class: text-center
---

<div class="space-y-8">
  <div v-click class="text-2xl">
    ¿Cuántos de ustedes, al escuchar sobre riesgo existencial,<br/>han pensado <span class="text-orange-400 font-bold italic">"esto suena a ciencia ficción"</span>?
  </div>

  <div v-click class="text-xl opacity-70 mt-8">
    Es completamente normal. Se llama sesgo de normalidad.
  </div>

  <div v-click class="text-lg opacity-70">
    Este riesgo es <span class="font-bold">invisible, gradual y difícil de entender</span>.<br/>
    La tormenta perfecta para la inacción.
  </div>

  <div v-click class="text-xl mt-8">
    Ustedes vivieron el amianto. El cambio climático.<br/>
    La diferencia: con la IA <span class="text-orange-400 font-bold">no tenemos 40 años</span>.<br/>
    Según los constructores, tenemos <span class="text-red-400 font-bold">entre 1 y 5</span>.
  </div>
</div>

<!--
"¿Cuántos de ustedes, al escuchar sobre riesgo existencial, han pensado 'esto suena a ciencia ficción'?"

[Pausa]

"Es completamente normal. Se llama sesgo de normalidad. Este riesgo es invisible, gradual y difícil de entender. La tormenta perfecta para la inacción.

Ustedes han vivido algo parecido. El amianto, el cambio climático. La diferencia con la IA es que no tenemos cuarenta años. Según los constructores, tenemos entre uno y cinco."
-->

---

# La carrera sin frenos

<div class="mt-8 space-y-6">
  <div v-click class="p-6 rounded-xl bg-white/5 border-l-4 border-orange-400">
    <div class="text-xl font-bold">El dilema del prisionero</div>
    <div class="mt-2 opacity-70">Si OpenAI para y Google no, Google gana. Aunque todos sepan que deberían parar, ninguno puede solo.</div>
  </div>
  <div v-click class="p-6 rounded-xl bg-white/5 border-l-4 border-orange-400">
    <div class="text-xl font-bold">EU AI Act: insuficiente</div>
    <div class="mt-2 opacity-70">Regula uso pero <span class="text-orange-400 font-bold">no restringe el entrenamiento</span>. Es como regular quién conduce pero no limitar la potencia del motor.</div>
  </div>
  <div v-click class="p-6 rounded-xl bg-white/5 border-l-4 border-red-400">
    <div class="text-xl font-bold">Cámara de los Lores, UK</div>
    <div class="mt-2 italic">"Una moratoria es, francamente, nuestra única esperanza."</div>
    <div class="opacity-50 text-sm mt-1">El gobierno no la respaldó. EEUU se negó a respaldar el Informe Internacional.</div>
  </div>
</div>

<!--
"Aunque todos los laboratorios supieran que deberían parar, ninguno puede solo. Si OpenAI para y Google no, Google gana. Es el dilema del prisionero.

El EU AI Act regula uso pero no restringe el entrenamiento. Es como regular quién conduce pero no limitar la potencia del motor.

En la Cámara de los Lores: 'Una moratoria es, francamente, nuestra única esperanza.' El gobierno no la respaldó. Y EEUU se negó a respaldar el Informe Internacional de Seguridad."
-->

---

# IA militar

<div class="mt-6 space-y-4">
  <div v-click class="p-4 rounded-xl bg-white/5">
    <div class="text-sm text-orange-400 font-bold">Junio 2025 · EEUU</div>
    <div class="mt-1"><span class="font-bold">Detachment 201</span> — CTO de Meta, CTO de Palantir, jefe de producto de OpenAI fichados como tenientes coroneles.</div>
  </div>
  <div v-click class="p-4 rounded-xl bg-white/5">
    <div class="text-sm text-orange-400 font-bold">Febrero 2026 · Venezuela</div>
    <div class="mt-1">Claude (IA de Anthropic) utilizada en la operación para capturar a Maduro.</div>
  </div>
  <div v-click class="p-4 rounded-xl bg-red-900/20 border border-red-800/50">
    <div class="text-sm text-red-400 font-bold">Marzo 2026 · Irán</div>
    <div class="mt-1">El ejército usó Claude para <span class="font-bold">identificar objetivos</span> en ataques a Irán.</div>
    <div class="opacity-70 text-sm mt-1">Horas después de que Trump la prohibiera. El Pentágono usó la herramienta mientras la prohibía.</div>
  </div>
</div>

<div v-click class="mt-4 text-center opacity-70">
  Anthropic dice que no permitirá armas autónomas.<br/>
  El Pentágono les dijo: <span class="text-orange-400 font-bold">o quitan sus salvaguardas, o los echamos.</span>
</div>

<!--
"Y mientras nadie frena, la IA ya se usa para matar.

Junio 2025: el ejército de EEUU creó el Detachment 201. Ficharon como tenientes coroneles al CTO de Meta, al CTO de Palantir, al jefe de producto de OpenAI.

Febrero 2026: Claude fue utilizada en la operación para capturar a Maduro en Venezuela.

Marzo 2026: el ejército usó Claude para identificar objetivos en los ataques a Irán. Horas después de que Trump la prohibiera. El Pentágono usó la herramienta mientras la prohibía.

Anthropic dice que no permitirá armas autónomas ni vigilancia masiva. Pero el Pentágono les dijo: o quitan sus salvaguardas, o los echamos."

TRANSICIÓN: "Entonces, ¿estamos condenados? No. Pero la ventana se cierra. Y lo que hagan ustedes importa más de lo que creen."
-->
